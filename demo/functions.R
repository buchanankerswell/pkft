  ...
  function(data,
    ...
    d <- data %>%
      ...
      mutate(
        ...
        d.sum = purrr::map2(data$age, data$ageSds, ~ tibble(
          x = seq(
            min(data$age, na.rm = TRUE) - sigrange * mean(data$ageSds, na.rm = TRUE),
            max(data$age, na.rm = TRUE) + sigrange * mean(data$ageSds, na.rm = TRUE),
            mean(data$ageSds, na.rm = TRUE) / 15
          ),
          d = dnorm(
            seq(
              min(data$age, na.rm = TRUE) - sigrange * mean(data$ageSds, na.rm = TRUE),
    ...
    if(nrow(d) == 1){
      ...
      d.n <- tibble(
        x = seq(
          min(d$age, na.rm = TRUE) - sigrange * mean(d$ageSds, na.rm = TRUE),
          max(d$age, na.rm = TRUE) + sigrange * mean(d$ageSds, na.rm = TRUE),
          sd(d$age, na.rm = TRUE) / 10
        ),
        d = dnorm(
          seq(
            min(d$age, na.rm = TRUE) - sigrange * mean(d$ageSds, na.rm = TRUE),
          ...
          mean = mean(d$age, na.rm = TRUE),
          sd = sd(d$age, na.rm = TRUE)
        )
      )
    }
    d.s <- d$d.sum %>%
    ...
    if (type == 'hist') {
      ...
      if('kernel' %in% curves) {
        p <- p + geom_density(data = d, aes(x = age, y = ..density..), color = 'black', fill = 'thistle4', alpha = 0.1)
      }
    } else if (type == 'point') {
      p <- d %>%
        ggplot() +
        ...
        coord_cartesian(xlim = xlim,
                        ylim = ylim) +
        scale_x_continuous(breaks = seq_along(d$age)) +
        theme_grey(base_size = 14) +
  ...
  function(pks,
    ...
    d <- pks$data %>%
      ...
      mutate(
        ...
        d.sum = purrr::map2(pks$data$age, pks$data$ageSds, ~ tibble(
          x = seq(
            min(pks$data$age, na.rm = TRUE) - sigrange * mean(pks$data$ageSds, na.rm = TRUE),
            max(pks$data$age, na.rm = TRUE) + sigrange * mean(pks$data$ageSds, na.rm = TRUE),
            mean(pks$data$ageSds, na.rm = TRUE) / 15
          ),
          d = dnorm(
            seq(
              min(pks$data$age, na.rm = TRUE) - sigrange * mean(pks$data$ageSds, na.rm = TRUE),
